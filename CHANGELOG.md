...
- Queries are sped up significantly.

## [1.3.0] - 2026-09-14

### Added

- Alembic schema versioning with serialized automatic container migrations.
- Configurable API documentation exposure, trusted proxy handling, and per-IP rate
  limiting.

### Changed

- The API now fails closed when its key is missing or insecure.
- API and webhook errors no longer expose internal exception details.
- Database schema creation is no longer available as an API or queued CLI job.
